...
Cypress.Commands.add("realizarCadastro", (dados = {}) => {
  ...
  cy.get('[data-testid="input-telefone"]').type(
    faker.helpers.replaceSymbols(usuario.telefone),
  );
...
Cypress.Commands.add("validarCamposLimpos", () => {
  ...
  cy.get('[data-testid="input-nascimento"]').should("have.value", "");
  cy.get('[data-testid="input-comentario"]').should("have.value", "");
  cy.get('[data-testid="input-senha"]').should("have.value", "");
});
